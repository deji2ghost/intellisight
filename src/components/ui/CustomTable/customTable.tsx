  ...
  return (
    <div className="flex flex-col justify-between h-[550px]">
      <div className="overflow-x-auto w-full">
        <table className="flex flex-col gap-5 w-full min-w-max table-fixed border-collapse">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              ...
              >
                {headerGroup.headers.map((header) => (
                  <th key={header.id} className="py-[7.5px] px-[10px] text-left min-w-[150px]">
                    {flexRender(
              ...
              table.getRowModel()?.rows?.map((row, index) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td key={cell.id} className="px-[10px] py-[10px] min-w-[150px]">
